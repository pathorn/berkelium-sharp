...
namespace Berkelium {
  namespace Managed {
    ...
    class URLStringHelper : public URLString {
      std::string mString;
    public:
      URLStringHelper(System::String ^ sysString) {
        pin_ptr<const wchar_t> pinnedData = PtrToStringChars(sysString);
        size_t length = sysString->Length;
        mString.resize(length);
        bool url_string_invalid = false;
        for (size_t i = 0; i < length; ++i) {
          if (pinnedData[i] <= 0 || pinnedData[i] >= 127) {
            url_string_invalid = true;
            mString[i] = '?';
          } else {
            mString[i] = static_cast<char>(pinnedData[i]);
          }
        }
        if (url_string_invalid) {
          // FIXME: Show an error?
        }
        this->mData = mString.data();
        this->mLength = mString.length();
      }
    };

    using ::Berkelium::WideString;
    class WideStringHelper : public WideString {
      std::wstring mString;
    public:
      WideStringHelper(System::String ^ sysString) {
        pin_ptr<const wchar_t> pinnedData = PtrToStringChars(sysString);
        size_t length = sysString->Length;
        mString = std::wstring(pinnedData, length);
        this->mData = mString.data();
        this->mLength = mString.length();
      }
    };

    using ::Berkelium::Cursor;
    ...
    public:
      ...
      void TextEvent (System::String ^ text) {
        WideStringHelper textPtr (text);

        Native->textEvent(textPtr.mData, textPtr.mLength);
      }
    ...
    public:
      ...
      virtual void onProvisionalLoadError(::Berkelium::Window *win, URLString url, int errorCode, bool isMainFrame);
      virtual void onConsoleMessage(::Berkelium::Window *win, WideString message,  WideString sourceId, int line_no);
      virtual void onScriptAlert(::Berkelium::Window *win, WideString message, WideString defaultValue, URLString url, int flags, bool &success, WideString &value);
      ...
      virtual void onExternalHost(::Berkelium::Window *win,  WideString message, URLString origin, URLString target);
      virtual void onCreatedWindow(::Berkelium::Window *win, ::Berkelium::Window *newWindow,  const ::Berkelium::Rect &initialRect);
      virtual void onPaint(::Berkelium::Window *win, const unsigned char *sourceBuffer, const ::Berkelium::Rect &sourceBufferRect,
          size_t numCopyRects, const ::Berkelium::Rect *copyRects, int dx, int dy, const ::Berkelium::Rect &scrollRect);
      ...
      virtual void onWidgetPaint(::Berkelium::Window *win, ::Berkelium::Widget *wid, const unsigned char *sourceBuffer, const ::Berkelium::Rect &sourceBufferRect,
          size_t numCopyRects, const ::Berkelium::Rect *copyRects, int dx, int dy, const ::Berkelium::Rect &scrollRect);
    ...
    public:
      ...
      virtual bool NavigateTo (System::String ^ url) {
        URLStringHelper urlPtr (url);
        return Native->navigateTo(urlPtr);
      }
      ...
      void ExecuteJavascript (System::String ^ javascript) {
        WideStringHelper scriptPtr (javascript);

        Native->executeJavascript(scriptPtr);
      }
      ...
      void InsertCSS (System::String ^ css, System::String ^ id) {
        WideStringHelper cssPtr (css);
        
        if (id == nullptr)
          Native->insertCSS(cssPtr, WideString::empty());
        else {
          WideStringHelper idPtr (id);
          Native->insertCSS(cssPtr, idPtr);
        }
      }
